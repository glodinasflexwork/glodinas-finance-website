...
export async function POST(request: NextRequest) {
  try {
    ...
    const notificationEmail = process.env.NOTIFICATION_EMAIL || 'glodinas@icloud.com';
    ...
    if (!response.ok) {
      let errorMessage = `Mailtrap API error: ${response.status} ${response.statusText}`;
      try {
        const errorData = await response.json();
        console.error('Mailtrap API error details:', errorData);
